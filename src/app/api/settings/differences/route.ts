...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create new business_differences record - use user.id as string directly
    const newRecord = await db.insert(businessDifferences)
      .values({
        userId: user.id,
        value: value.trim(),
        createdAt: new Date(),
    ...
    try {
      ...
      const webhookBody = `business_difference: ${value.trim()}`;
